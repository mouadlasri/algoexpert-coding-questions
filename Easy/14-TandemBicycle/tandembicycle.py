...
#   5, and person B pedals at a speed of 4, the tandem bicycle moves at a speed
...
def tandemBicycle(redShirtSpeeds, blueShirtSpeeds, fastest):
    ...
    if(fastest==True):
        # adding the max red with min blue to get the maximum total speed
        blueShirtSpeeds.sort(reverse=True) # or can use blueShirtSpeeds.sort(reverse = fastest)
    else:
        # adding the max red with the max to get the minimum total speed
        blueShirtSpeeds.sort() 
    
    for index, shirt in enumerate(redShirtSpeeds):
        totalMaxMinSpeed += max(redShirtSpeeds[index], blueShirtSpeeds[index])
